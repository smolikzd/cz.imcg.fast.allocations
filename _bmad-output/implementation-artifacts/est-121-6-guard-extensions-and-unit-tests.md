...
- [ ] Task 15: Extend EST-102 duplicate check to block *_REQUESTED statuses (AC: #1, #2)
  - [ ] Open `src/zcl_fi_process_manager.clas.abap`
  - [ ] Find `create_process()` duplicate check — the `line_exists()` chain (after EST-110 changes)
  - [ ] Current: checks RUNNING, FAILED, COMPLETED
  - [ ] Add: `line_exists( lt_existing[ status = gc_status-exec_requested ] )` and `line_exists( lt_existing[ status = gc_status-restart_requested ] )`
  - [ ] Update the error message value string to include new statuses
  - [ ] Break each `line_exists()` onto its own line to stay under 120 chars

- [ ] Task 16: Extend EST-106 parallel limit to count *_REQUESTED statuses (AC: #3)
  - [ ] Find the parallel instance count query (EST-106 implementation: `check_parallel_limit()` or similar)
  - [ ] Current: counts WHERE status = 'RUNNING' (or uses a single-value check)
  - [ ] Replace with RANGE table approach:
    ```
    ...
    ```
  - [ ] Adapt to actual EST-106 implementation pattern (the above is illustrative)

- [ ] Task 17: Unit tests for new methods (AC: #4-#11)
  - [ ] Open `src/zcl_fi_process_manager.clas.testclasses.abap`
  - [ ] Add test methods (guard-only — no actual APJ scheduling):
  ...
  - [ ] Follow existing test patterns: `FOR TESTING DURATION SHORT RISK LEVEL HARMLESS`, Given/When/Then with `cl_abap_unit_assert`
  - [ ] Clean up test data in teardown (DELETE zfi_proc_inst + COMMIT WORK AND WAIT)
